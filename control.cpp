        ...
        switch(mission_state){
            ...
            case SEARCHING:
            {
                ...
                if(searching_index >= searching_points.size() || searching_index < 0) {
                    ...
                    if(target_pose.pose.position.z != -1) mission_state = BOMB_NAVIGATING;
                        ...
                        ROS_WARN("All searching points reached, and this should not happen. Returning now.");
            ...
            case OBSTACLE_AVOIDING: // TODO: 这里可以添加一个类似投弹后的adjusting状态，以保证精准降落
            {
                if(!is_param_set){
                    ...
                    nh.setParam("manager/a_star_time_limit", 0.5);
                    nh.setParam("optimization/lambda_smooth", 250);
                    nh.setParam("optimization/lambda_collision", 700);
