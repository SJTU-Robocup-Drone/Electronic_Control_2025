...
def detect_targets():
    ...
    try:
        ...
        rospy.loginfo("[INFO] TensorRT模型加载完成。")
    except Exception as e:
        rospy.logerr(f"TensorRT模型加载失败: {e}")
        return

    # 类别名称需要单独定义（因为TensorRT引擎不包含这些信息）
    ...
    try:
        while scanning_active and not stop_event.is_set() and not rospy.is_shutdown():
            ...
            img = img.astype(np.float32) / 255.0  # 修改：确保数据类型为float32

            # 修改：使用TensorRT推理
            try:
                output = model.infer(img)

                # 将TensorRT输出转换为与原始YOLO输出兼容的格式
                # 这里需要根据您的具体模型输出格式进行调整
                pred = torch.from_numpy(output).reshape(1, -1, 12)  # 7类别模型，应该是7+5=12维
                pred = non_max_suppression(pred, conf_thres=0.25, iou_thres=0.45)[0]

            except Exception as e:
                rospy.logerr(f"TensorRT推理失败: {e}")
                continue
