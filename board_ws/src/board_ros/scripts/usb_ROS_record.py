...
video_save_path = "/home/amov/board_ws/src/board_ros/videos/"  # 视频保存路径
...
def init_video_writer(width, height, fps):
    ...
    video_filepath = os.path.join(video_save_path, video_filename)
    ...
    if video_writer.isOpened():
        rospy.loginfo(f"[INFO] 视频录制已开始，保存路径: {video_filepath}")
        return True
    else:
        rospy.logerr("[ERROR] 无法初始化视频写入器")
        return False
...
def detect_targets():
    ...
    # 初始化视频录制
    recording_active = init_video_writer(desired_width, desired_height, 30)  # 录制帧率设为30fps
    ...
    try:
        while scanning_active and not stop_event.is_set() and not rospy.is_shutdown():
            ...
            if pred is not None and len(pred):
                ...
                # 如果需要保存带检测框的视频，可以替换为frame_with_boxes
                # if recording_active and video_writer is not None:
...
```我来帮您在现有的 `usb_ROS.py` 文件中添加视频录制功能。以下是增强后的代码，可以将摄像头捕捉到的视频画面保存下来：

```python name=usb_ROS.py
#!/home/amov/miniconda3/envs/yolo/bin/python
