...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    {
      waypoint_sub_ = nh.subscribe("/control/move_base_simple/goal", 1, &EGOReplanFSM::waypointCallback, this);
    }
  ...
  void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    if (success)
    {
      ...
      /*** FSM ***/
      if (exec_state_ == WAIT_TARGET || exec_state_ == ESCAPING)
        changeFSMExecState(GEN_NEW_TRAJ, "TRIG");
      else
      {
  ...
  void EGOReplanFSM::odometryCallback(const nav_msgs::OdometryConstPtr &msg)
  {
    ...
    if(++odom_cnt >= 10 && odom_pos_(2) >= 0.4 && odom_pos_(2) <= 1.5 && !escape_state_msg_.data){
      odom_cnt = 0;
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case ESCAPING: {
      ...
      if (is_in_inflated_zone_ != 1) { // 已成功逃离膨胀区，正常规划轨迹
        ROS_INFO("Successfully escaped inflated zone. Planning to original target.");
        if(has_original_target_) planNextWaypoint(original_target_);
